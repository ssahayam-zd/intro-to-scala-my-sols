...
object EitherExercises {
  ...
  def createPersonAndShow(name: String, age: String): String = {
    createPerson(name, age) match {
      ...
      case Left(InvalidAgeRange(age)) => s"Provided age must be between 1-120: $age"
      // case Right(Person(n, a)) => s"$n is $age"
      case Right(person) => s"${person.name} is ${person.age}"
    }
  }
  ...
  def createValidPeople: List[Person] = {
    val maybePeopleList: List[Either[AppError, Person]] = personStringPairs.map {
      case (name, age) => createPerson(name, age)
    }

    maybePeopleList.collect {
